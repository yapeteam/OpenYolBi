...
public class ImplScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (OpenGlHelper.shadersSupported && mc.thePlayer != null && guiModuleInstance.getBlur().getValue()) {
            ShaderGroup theShaderGroup = ReflectionManager.GetEntityRenderer$theShaderGroup(mc.entityRenderer);
            if (theShaderGroup != null)
                theShaderGroup.deleteShaderGroup();
            ReflectionManager.EntityRenderer$loadShader(mc.entityRenderer, new ResourceLocation("shaders/post/blur.json"));
            int radius = guiModuleInstance.getBlurRadius().getValue();
            theShaderGroup = ReflectionManager.GetEntityRenderer$theShaderGroup(mc.entityRenderer);
            List<Shader> listShaders = ReflectionManager.GetShaderGroup$listShaders(theShaderGroup);
